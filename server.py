    ...
    def watchdog():
        while True:
            ...
            if client_info['addr'] and (time.time() - client_info['last_contact'] > 5):
                print(f"[Watchdog] 客户端 {client_info['addr']} 超时，停止推流。")
                manager.stop_stream()
...
def stream_from_camera(video_sock, audio_sock, controller, running_flag, client_addr):
    ...
    try:
        ...
        print(f"[Streamer-Audio] 无法打开麦克风: {e}")
        p_audio.terminate()
        return
...
def main():
    ...
    print(f"[Server] 服务器已启动于 {SERVER_HOST}:{CONTROL_PORT}。按 Ctrl+C 关闭。")
    try:
        control_thread.join()
